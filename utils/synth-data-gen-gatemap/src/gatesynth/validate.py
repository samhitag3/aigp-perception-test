...
def validate_dataset(root: str | Path) -> dict:
    ...
    for seqdir in sorted((root / "sequences").glob("seq_*")):
        ...
        outcome = sm.get("outcome", motion.get("outcome", {}))
        status = str(outcome.get("status", "success"))
        if status not in {"success", "failure"}:
            errors.append(f"{seqdir.name}: invalid outcome status {status}")
        else:
            outcome_counts[status] = outcome_counts.get(status, 0) + 1

        failure_target = outcome.get("failure_target_track_id")
        if status == "failure":
            if not outcome.get("intentional_failure", False):
                errors.append(f"{seqdir.name}: failure sequence is not marked intentional_failure")
            if not failure_target:
                errors.append(f"{seqdir.name}: failure sequence missing failure_target_track_id")
            else:
                failure_target_counts[failure_target] = failure_target_counts.get(failure_target, 0) + 1
            if motion.get("completed_course") is not False:
                errors.append(f"{seqdir.name}: intentional failure must have completed_course=false")
        else:
            if outcome.get("intentional_failure", False):
                errors.append(f"{seqdir.name}: success sequence marked intentional_failure")
            if motion.get("completed_course") is not True:
                errors.append(f"{seqdir.name}: success sequence did not complete the course")

        if motion.get("truncated_by_max_frames", False):
            errors.append(f"{seqdir.name}: trajectory hit max_frames_per_sequence")

        gate_tracks = sm.get("course", {}).get("gate_tracks", [])
        track_ids = {g["track_id"] for g in gate_tracks}
        route_gates = sorted(
            [
                g for g in gate_tracks
                if g.get("is_route_target", True)
                and g.get("route_order_index") is not None
            ],
            key=lambda g: int(g["route_order_index"]),
        )
        route_indices = [int(g["route_order_index"]) for g in route_gates]

        if len(route_indices) != len(set(route_indices)):
            errors.append(f"{seqdir.name}: duplicate route_order_index")

        # Route indices should be contiguous 0..N-1 for this fixed course.
        if route_indices and route_indices != list(range(route_indices[0], route_indices[0] + len(route_indices))):
            errors.append(f"{seqdir.name}: route_order_index has gaps: {route_indices}")

        reached_route_gates = [g for g in route_gates if g.get("path_s_m") is not None]
        route_path_s = [float(g["path_s_m"]) for g in reached_route_gates]
        if len(route_path_s) >= 2 and not all(b > a for a, b in zip(route_path_s[:-1], route_path_s[1:])):
            errors.append(f"{seqdir.name}: gate path_s_m is not strictly increasing")

        if status == "success" and len(reached_route_gates) != len(route_gates):
            errors.append(f"{seqdir.name}: success sequence has route gates without path_s_m")

        if status == "failure" and failure_target:
            reached_ids = [g["track_id"] for g in reached_route_gates]
            if failure_target not in reached_ids:
                errors.append(f"{seqdir.name}: failure target {failure_target} was not reached in path metadata")
            else:
                fail_pos = reached_ids.index(failure_target)
                if fail_pos != len(reached_ids) - 1:
                    errors.append(f"{seqdir.name}: path continues to route gates after failure target {failure_target}")
    ...
    plan_path = root / "outcome_plan.json"
    if plan_path.exists():
        plan = _load_json(plan_path)
        expected_counts = plan.get("counts", {})
        if int(expected_counts.get("success", outcome_counts.get("success", 0))) != int(outcome_counts.get("success", 0)):
            errors.append(
                f"outcome count mismatch for success: expected {expected_counts.get('success')} "
                f"got {outcome_counts.get('success', 0)}"
            )
        for target, expected in expected_counts.items():
            if target == "success":
                continue
            actual = int(failure_target_counts.get(target, 0))
            if int(expected) != actual:
                errors.append(f"outcome count mismatch for {target}: expected {expected} got {actual}")
    ...
    return {
        ...
        "failure_target_counts": failure_target_counts,
        "errors": errors[:200],
