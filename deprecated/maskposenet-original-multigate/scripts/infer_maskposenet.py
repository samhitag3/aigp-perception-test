#!/usr/bin/env python3
"""MaskPoseNet-MG inference from per-gate instance-mask sequences.

Model input is ONLY the mask sequence.  --rgb-input is visualization-only.
Folder input: prefer uint16/uint8 label PNGs (0=background, 1..N=gate instances).
Video input: supports grayscale labels or distinct non-black colors per instance.
"""
...
def alpha_blend_mask(image, mask, bgr, alpha):
    """Alpha-blend a solid BGR color into masked pixels."""
    if not np.any(mask):
        return
...
def labels_to_model(labels,h,w):
    ...
    for i,mid in enumerate(ids): rgb[labels==mid]=PALETTE[i%len(PALETTE)]
...
def main():
    ...
    ap.add_argument('--input',required=True,help='instance-mask folder or mask video')
    ...
    ap.add_argument('--presence-th',type=float,default=.5); ap.add_argument('--mask-th',type=float,default=.5); ap.add_argument('--overlay-alpha',type=float,default=.35)
    ...
    for idx,(labels,name) in enumerate(iter_source(args.input)):
        ...
        keep=np.where(pres>=args.presence_th)[0].tolist()
        ...
        for j,q in enumerate(keep):
            bgr = gate_bgr(j)
            pm=F.interpolate(masks[q][None,None],size=(H,W),mode='bilinear',align_corners=False)[0,0].numpy()>=args.mask_th
            pred_id[pm]=j+1
            alpha_blend_mask(
                overlay,
                pm,
