...
class CanonicalGateSequenceDatasetMG(Dataset):
    ...
    def _load_frame(self, rec: _MGFrame, color_map: Optional[dict[int, np.ndarray]] = None) -> dict[str, torch.Tensor]:
        h, w = self.size
        W0, H0 = rec.native_wh
        ...
        if self.input_mode == "instance_color":
            ...
            # the model input. Every non-zero gate pixel becomes foreground=1,
            # rendered identically in all three channels so the encoder shape
            # remains [3,H,W]. IMPORTANT: `mask_ids` itself is left untouched
            # below, so per-gate GT masks/keypoints/pose supervision remains
            # exactly the same as MaskPoseNet.
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        # The union-input variant does not encode instance identity/color.
        # `color_map` is therefore intentionally unused.
        frames = [self._load_frame(self._frames[j], color_map=None) for j in padded]
