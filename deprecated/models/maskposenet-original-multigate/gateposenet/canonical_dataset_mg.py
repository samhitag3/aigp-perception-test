...
class CanonicalGateSequenceDatasetMG(Dataset):
    ...
    def _load_frame(self, rec: _MGFrame, color_map: Optional[dict[int, np.ndarray]] = None) -> dict[str, torch.Tensor]:
        ...
        if self.input_mode == "instance_color":
            if mask_ids is None:
                raise FileNotFoundError(rec.instance_mask_path)
            rgb = np.zeros((*mask_ids.shape, 3), dtype=np.uint8)
            palette = self._palette()
            ids = [int(x) for x in np.unique(mask_ids) if int(x) > 0]
            for j, mid in enumerate(ids):
                color = color_map.get(mid) if color_map is not None and mid in color_map else palette[(mid - 1) % len(palette)]
                rgb[mask_ids == mid] = color
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        color_map = None
        if self.input_mode == "instance_color":
            mids = sorted({int(g.mask_id) for j in padded for g in self._frames[j].gates if g.mask_id is not None and int(g.mask_id) > 0})
            palette = self._palette().copy()
            if self.randomize_instance_colors and len(mids) > 1:
                perm = torch.randperm(len(palette)).tolist()
                palette = palette[perm]
            color_map = {mid: palette[k % len(palette)] for k, mid in enumerate(mids)}
        frames = [self._load_frame(self._frames[j], color_map=color_map) for j in padded]
