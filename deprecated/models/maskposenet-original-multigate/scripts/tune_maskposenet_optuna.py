#!/usr/bin/env python3
"""Optuna tuning for MaskPoseNet-MG using the fixed canonical 50% subset."""
...
def main():
    ...
    def objective(trial):
        ...
        window_choices = list(tune.get("window_choices", [4, 6, 8, 10, 12, 16]))
        cfg["data"]["window"] = trial.suggest_categorical("window", window_choices)
        cfg["data"]["stride"] = min(int(cfg["data"].get("stride", 4)), int(cfg["data"]["window"]))
        cfg["data"]["val_stride"] = int(cfg["data"]["window"])
        for key, lo, hi in [
        ...
        lat = benchmark_step(
            model, device, int(cfg["data"].get("height", 192)), int(cfg["data"].get("width", 320)),
            int(tune.get("latency_warmup", 40)), int(tune.get("latency_iters", 120)))
        trial.set_user_attr("step_latency", lat)
        max_ms = float(tune.get("max_5090_step_ms", 2.5))
        if lat["p95_ms"] > max_ms:
            raise optuna.TrialPruned(f"p95 step latency {lat['p95_ms']:.3f} ms > {max_ms:.3f} ms proxy budget")
        ...
        # Stateful deployment latency is independent of training-window length;
        # use a tiny tie-break reward so, all else equal, Optuna prefers more history.
        window_reward = float(tune.get("window_tiebreak_reward", 1e-4))
        return float(best) - window_reward * float(cfg["data"]["window"])
    ...
    for k, v in bp.items():
        if k in {"lr", "weight_decay", "grad_clip"}:
            best_cfg["train"][k] = v
        elif k in {"ego_dropout", "window"}:
            best_cfg["data"][k] = v
            if k == "window":
                best_cfg["data"]["val_stride"] = int(v)
                best_cfg["data"]["test_stride"] = int(v)
        else:
            best_cfg["loss"][k] = v
