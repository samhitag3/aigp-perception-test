...
from gateperception.training.evaluate_models import evaluate_keypoint_model
import json
...
def main():
    ...
    _, _, train_loader, val_loader = build_keypoint_loaders(cfg)
    ...
    result = train_loop(model, train_loader, val_loader, opt, int(cfg["training"]["epochs"]), device, run_dir, cfg, keypoint_step, keypoint_val, amp=bool(cfg["training"].get("amp", True)), scheduler=sched)
    best = torch.load(run_dir / "best.pt", map_location="cpu"); model.load_state_dict(best["model"]); model.to(device)
    metrics = evaluate_keypoint_model(model, val_loader, device, cfg)
    report = {"schema_version":"1.0.0","model_name":"temporal_mask_keypoint_net","evaluation_split":"validation","training":{"best_epoch":best["epoch"],"best_val_metric":best["best_metric"],"training_time_seconds":result["training_time_seconds"]},"evaluation":metrics}
