...
def main():
    ...
    ap.add_argument("--output",default="evaluation_test.json")
    ap.add_argument("--per-sample-output")
    args=ap.parse_args()
    ...
    ds,loader=build_loader(cfg,args.split,False); model=GatePerceiver(cfg); model.load_state_dict(ck["model"]); model.to(dev)
    m,rows=evaluate_model(model,loader,cfg,dev,return_rows=True); score=selection_score(m,cfg)
    report=build_evaluation_report(cfg,ds.dataset_meta,m,model,score,args.split); write_json(report,args.output)
