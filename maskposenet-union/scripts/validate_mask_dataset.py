#!/usr/bin/env python3
"""Validate canonical UAV gate dataset for MaskPoseNet (instance masks are the required visual input)."""
...
def main():
    ...
    for sid in sequence_ids:
        ...
        for line_no, fr in read_jsonl(frames_jsonl):
            ...
            if not mask_rel:
                errors.append(f"{sid}:{line_no}: files.instance_mask missing")
                continue
            mask_path = seq_dir / mask_rel
            m0 = cv2.imread(str(mask_path), cv2.IMREAD_UNCHANGED)
            if m0 is None:
            ...
            H, W = m0.shape[:2]
            n_masks += 1
            image_meta = fr.get("image") or {}
            expected_w = int(image_meta.get("width_px") or cam.get("width_px") or args.width)
            expected_h = int(image_meta.get("height_px") or cam.get("height_px") or args.height)
            if (W, H) != (expected_w, expected_h):
                errors.append(f"{sid}:{line_no}: instance mask is {W}x{H}, metadata says {expected_w}x{expected_h}")
            ...
            ids = set(int(x) for x in np.unique(m0) if int(x) != 0)
