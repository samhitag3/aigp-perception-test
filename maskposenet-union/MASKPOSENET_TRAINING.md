...
## Post-training presence-threshold Optuna calibration

Presence threshold is a post-processing calibration parameter, not a training
hyperparameter. Run the model hyperparameter study first, train the final model,
then tune presence threshold on validation only. The tuner caches model outputs
once, so Optuna trials do not rerun the network.
...
The generated `best_config_with_threshold.yaml` stores the calibrated threshold
under `inference.presence_threshold`. Both evaluation and inference will use it
by default unless `--presence-th` is supplied explicitly.
