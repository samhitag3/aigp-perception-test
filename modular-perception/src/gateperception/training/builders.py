...
def build_seg_loaders(cfg: dict[str, Any]):
    ...
    tr = DataLoader(train_ds, batch_size=int(cfg["training"]["batch_size"]), shuffle=True, generator=g, num_workers=int(cfg["training"].get("num_workers", 4)), pin_memory=True, collate_fn=collate_seg)
...
def build_keypoint_loaders(cfg: dict[str, Any]):
    ...
    g = torch.Generator().manual_seed(int(cfg.get("seed", 42)))
    tr = DataLoader(train_ds, batch_size=int(cfg["training"]["batch_size"]), shuffle=True, generator=g, num_workers=int(cfg["training"].get("num_workers", 4)), pin_memory=True, collate_fn=collate_keypoints)
