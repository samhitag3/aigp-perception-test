...
def sequence_camera(index: CanonicalIndex, sid: str, row: dict, out_w: int, out_h: int, fallback_cam: dict) -> tuple[np.ndarray, np.ndarray]:
    seq_meta_path = index.root / "sequences" / sid / "sequence.json"
    meta = {}
    if seq_meta_path.exists():
        try:
            meta = json.loads(seq_meta_path.read_text())
        except Exception:
            meta = {}
    cam = meta.get("camera", {})
    intr = cam.get("intrinsics", {})
    if not intr:
        intr = row.get("camera", {}).get("intrinsics", {})
    src_w = int(cam.get("width_px") or row.get("image", {}).get("width_px") or out_w)
    src_h = int(cam.get("height_px") or row.get("image", {}).get("height_px") or out_h)
    Ksrc = intr.get("K")
    if Ksrc is not None:
        Ksrc = np.asarray(Ksrc, dtype=np.float64)
        fx0, fy0, cx0, cy0 = float(Ksrc[0,0]), float(Ksrc[1,1]), float(Ksrc[0,2]), float(Ksrc[1,2])
    else:
        fx0, fy0 = float(intr.get("fx", fallback_cam["fx"])), float(intr.get("fy", fallback_cam["fy"]))
        cx0, cy0 = float(intr.get("cx", fallback_cam["cx"])), float(intr.get("cy", fallback_cam["cy"]))
    fx, fy, cx, cy = fx0, fy0, cx0, cy0
    sx, sy = out_w / max(1, src_w), out_h / max(1, src_h)
    K = camera_matrix(fx * sx, fy * sy, cx * sx, cy * sy)
    dist = np.asarray(fallback_cam.get("distortion_coefficients", [0,0,0,0,0]), dtype=np.float64)
    return K, dist
...
def main():
    ...
    fallback_cam = load_yaml(args.camera_config)["camera"]
    geom = load_yaml(args.gate_geometry)["gate"]
    W = int(scfg.get("dataset", {}).get("width", fallback_cam["width_px"]))
    H = int(scfg.get("dataset", {}).get("height", fallback_cam["height_px"]))
    ...
    for sid in index.sequence_ids:
        ...
        (seq_out / "instance_masks").mkdir(parents=True, exist_ok=True)
        ...
        with open(seq_out / "frames.jsonl", "w", encoding="utf-8") as fout:
            for row in index.sequences[sid]:
                ...
                src_h, src_w = rgb_src.shape[:2]
                rgb_np = rgb_src if (src_w, src_h) == (W, H) else cv2.resize(rgb_src, (W, H), interpolation=cv2.INTER_AREA)
                K, distortion = sequence_camera(index, sid, row, W, H, fallback_cam)
                ...
                Image.fromarray(inst.astype(np.uint16)).save(seq_out/"instance_masks"/name)
                pred = {"sequence_id":sid,"frame_index":int(row["frame_index"]),"timestamp_s":row.get("sim_time_s"),"instance_mask":f"instance_masks/{name}","prediction_width_px":W,"prediction_height_px":H,"source_width_px":src_w,"source_height_px":src_h,"num_gate_instances":len(instances),"num_mask_instances":sum(1 for x in instances if x["mask_id"] is not None),"instances":instances,"runtime":{"inference_ms":(t1-t0)*1000,"postprocess_and_downstream_ms":(t2-t1)*1000,"total_ms":(t2-t0)*1000}}
