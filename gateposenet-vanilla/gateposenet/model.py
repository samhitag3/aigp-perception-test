...
class GatePoseNetMG(nn.Module):
    ...
    def __init__(
        ...
        # pixel decoder to 1/4 res (mask features) — reuses encoder skips
        self.px_up1 = Up(c5, c4)
        self.px_up2 = Up(c4, c3)
        self.mask_feat = nn.Conv2d(c3, d_model, kernel_size=1)

        # query transformer decoder over the GRU state tokens
    ...
    def _decode(self, h, x3, x4):
        B, C, Hs, Ws = h.shape
        tokens = self.in_proj(h).flatten(2).transpose(1, 2)   # (B, HW, D)
        tokens = tokens + _sine_pos_2d(self.d_model, Hs, Ws,
                                       h.device)[None]
        ...
        # MaskFormer-style instance masks at 1/4 resolution
        px = self.mask_feat(self.px_up2(self.px_up1(h, x4), x3))  # (B,D,h4,w4)
