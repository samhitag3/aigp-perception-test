#!/usr/bin/env python3
"""Train unchanged MaskPoseNet-MG directly on canonical contract datasets."""
...
def main():
    ...
    noise_cfg = cfg.get("data", {}).get("mask_noise", {})
    noise_on = bool(noise_cfg.get("enabled", False))
    print(f"device={device} train_windows={len(train_ds)} val_windows={len(val_ds)} "
          f"max_gates={max_gates} queries={n_queries} train_mask_noise={noise_on}")
    model = build_gateposenet_mg(cfg["model"]).to(device)
    print(f"MaskPoseNet-MG parameters: {model.num_parameters()/1e6:.3f} M")
