...
def build_mg_dataset(cfg: dict, split: str, ego_dropout: float = 0.0):
    ...
    mask_stride = int(cfg.get("model", {}).get("mask_stride", d.get("mask_stride", 4)))
    if "mask_stride" in d and int(d["mask_stride"]) != mask_stride:
        raise ValueError("data.mask_stride must match model.mask_stride")
    preserve_small_masks = bool(d.get("preserve_small_masks", False))
    datasets = [CanonicalGateSequenceDatasetMG(
        ...
        mask_stride=mask_stride, preserve_small_masks=preserve_small_masks,
        input_mode=str(d.get("input_mode", "rgb")),
        randomize_instance_colors=bool(d.get("randomize_instance_colors", False) and split == "train"),
        # Mask corruption is deliberately TRAIN-ONLY. Validation/test always
        # use the original clean union masks and original GT supervision.
        mask_noise_cfg=(d.get("mask_noise", {}) if split == "train" else None),
    ) for s in specs]
    if len(datasets) == 1:
        return datasets[0]
...
@torch.no_grad()
def validate_mg(model, loader, criterion, device, presence_th: float = 0.5, mask_th: float = 0.5):
    model.eval()
    lm = AverageMeter()
    ...
    for batch in loader:
        ...
        lm.update(loss.detach().item(), n=batch["image"].shape[0])
        ...
        n_pred = (prob >= float(presence_th)).sum(dim=2).float()
        n_gt = batch["g_valid"].sum(dim=2)
        valid_frames = batch["frame_ok"] > 0.5
        if valid_frames.any():
            pres_ok.append((n_pred == n_gt)[valid_frames].float().cpu())

        # Hungarian association lets us measure per-GT detection recall even
        # when several gates coexist.  This is the key metric for far gates.
        assign = criterion._match(pred, batch)
        safe = assign.clamp_min(0)
        matched_prob = prob.gather(2, safe)
        visible_gt = ((batch["g_valid"] > 0.5) &
                      (batch["g_visible"] > 0.5) &
                      (batch["frame_ok"][..., None] > 0.5) &
                      (assign >= 0))
        detected = matched_prob >= float(presence_th)
        gate_total += int(visible_gt.sum())
        gate_hits += int((visible_gt & detected).sum())
        ranges = batch["g_position"].norm(dim=-1)
        far_gt = visible_gt & (ranges >= far_start)
        far_total += int(far_gt.sum())
        far_hits += int((far_gt & detected).sum())

        pm = torch.sigmoid(pred["mask_logit"]) >= float(mask_th)
        # matched per-GT mask IoU, especially important for tiny far masks
        idx = safe[..., None, None].expand(-1, -1, -1, pm.shape[-2], pm.shape[-1])
        pm_match = pm.gather(2, idx)
        gm_all = batch["g_mask"] > 0.5
        inter_m = (pm_match & gm_all).sum(dim=(-1, -2)).float()
        union_m = (pm_match | gm_all).sum(dim=(-1, -2)).float().clamp_min(1.0)
        miou = inter_m / union_m
        far_mask_valid = far_gt & (gm_all.sum(dim=(-1, -2)) > 0)
        if far_mask_valid.any():
            far_iou_sum += float(miou[far_mask_valid].sum())
            far_iou_n += int(far_mask_valid.sum())
        ...
        for b in range(pm.shape[0]):
            for tt in range(pm.shape[1]):
                ...
                for g in range(batch["g_valid"].shape[2]):
                    if batch["g_valid"][b, tt, g] < 0.5 or batch["g_mask"][b, tt, g].sum() < 1:
                        continue
    ...
    out["gate_recall"] = gate_hits / gate_total if gate_total else 0.0
    out["far_gate_recall"] = far_hits / far_total if far_total else 0.0
    out["far_mask_iou"] = far_iou_sum / far_iou_n if far_iou_n else 0.0
    return out
    ...
    try:
        for epoch in range(epochs):
            ...
            for batch in tqdm(train_loader, desc=f"[{tag}] epoch {epoch+1}/{epochs}", leave=False):
                ...
                meter.update(loss.detach().item(), n=batch["image"].shape[0]); step += 1
            ...
            if selection.get("metric", "target_pos_err_m") == "far_balanced_score":
                metrics["far_balanced_score"] = (
                    metrics["target_pos_err_m"]
                    + float(selection.get("far_recall_weight_m", 0.75)) * (1.0 - metrics["far_gate_recall"])
                    + float(selection.get("gate_recall_weight_m", 0.25)) * (1.0 - metrics["gate_recall"])
                )
                sel = metrics["far_balanced_score"]
            else:
            ...
            print(f"[{tag}] epoch {epoch+1}/{epochs} train_loss={meter.avg:.4f} "
                  ...
                  f"mask_iou={metrics['mask_iou']:.3f} recall={metrics['gate_recall']:.3f} "
                  f"far_recall={metrics['far_gate_recall']:.3f} far_iou={metrics['far_mask_iou']:.3f} "
                  f"select={sel:.4f} ({time.time()-t0:.0f}s)")
