...
from .mask_noise import WindowMaskNoise, mask_noise_enabled
...
class CanonicalGateSequenceDatasetMG(Dataset):
    ...
    def __init__(
        ...
        input_mode: str = "rgb",
        randomize_instance_colors: bool = False,
        mask_noise_cfg: Optional[dict[str, Any]] = None,
    ):
        ...
        self.randomize_instance_colors = bool(randomize_instance_colors)
        self.mask_noise_cfg = dict(mask_noise_cfg or {})
        self.mask_noise_enabled = mask_noise_enabled(self.mask_noise_cfg)
        if self.input_mode not in {"rgb", "instance_color"}:
            raise ValueError("input_mode must be rgb or instance_color")
        if self.max_gates < 1:
    ...
    def _add_sequence(self, sequence_id: str) -> None:
        ...
        for i, frame in enumerate(raw_frames):
            ...
            if self.input_mode == "rgb" and not rgb_rel:
                continue
            if self.input_mode == "instance_color" and not mask_rel:
                continue
            rgb_path = (seq_dir / rgb_rel) if rgb_rel else None
    ...
    def _palette() -> np.ndarray:
        # Bright, well-separated RGB colors.  Zero remains background.
        return np.asarray([
    ...
    def _load_frame(self, rec: _MGFrame, color_map: Optional[dict[int, np.ndarray]] = None,
                    mask_noise: Optional[WindowMaskNoise] = None, frame_pos: int = 0) -> dict[str, torch.Tensor]:
        h, w = self.size
        W0, H0 = rec.native_wh
        ...
        if rec.instance_mask_path is not None and rec.instance_mask_path.exists():
            ...
            if mask_ids.ndim == 3:
                mask_ids = mask_ids[..., 0]
            H_img, W_img = mask_ids.shape[:2]
            if (W_img, H_img) != (W0, H0):
                W0, H0 = W_img, H_img

        if self.input_mode == "instance_color":
            if mask_ids is None:
                raise FileNotFoundError(rec.instance_mask_path)

            # UNION-INPUT VARIANT: instance IDs are deliberately discarded for
            # the model input. During TRAINING ONLY, `mask_noise` may corrupt a
            # copy of those per-instance masks before unionization. `mask_ids`
            # itself is NEVER modified, so the GT masks/keypoints/pose below and
            # all validation/test metrics remain clean/original.
            if mask_noise is not None:
                union = mask_noise.apply(mask_ids, frame_pos=frame_pos)
            else:
                union = (mask_ids > 0).astype(np.uint8) * 255
            rgb = np.repeat(union[..., None], 3, axis=2)

            img_r = cv2.resize(rgb, (w, h), interpolation=cv2.INTER_NEAREST)
            image = torch.from_numpy(img_r.transpose(2, 0, 1)).float() / 255.0
        else:
            if rec.rgb_path is None:
                raise FileNotFoundError("RGB input requested but frame has no rgb path")
            ...
            image = torch.from_numpy(cv2.cvtColor(img_r, cv2.COLOR_BGR2RGB).transpose(2, 0, 1)).float() / 255.0
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        # Training-only mask corruption is sampled ONCE per temporal window,
        # then varied mildly across its frames. This lets the ConvGRU learn to
        # bridge persistent partial masks, dropouts and false detections.
        mask_noise = None
        if self.mask_noise_enabled:
            seed = int(torch.randint(0, 2**31 - 1, ()).item())
            mask_noise = WindowMaskNoise(self.mask_noise_cfg, seed=seed)
        frames = [
            self._load_frame(self._frames[j], color_map=None,
                             mask_noise=mask_noise, frame_pos=t)
            for t, j in enumerate(padded)
        ]
