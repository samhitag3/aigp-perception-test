> **Union + realistic mask-noise variant:** this repo can train MaskPoseNet from unionized masks with training-only temporally correlated segmentation corruption. Validation/test remain clean. See `UNION_NOISE_TRAINING.md` and `scripts/preview_mask_noise.py`.

> **MaskPoseNet variant:** see `MASKPOSENET_TRAINING.md` for mask-only training, evaluation, latency benchmarking, and inference.
