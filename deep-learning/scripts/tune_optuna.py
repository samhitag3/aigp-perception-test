...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--config",required=True); ap.add_argument("--dataset-root"); ap.add_argument("--run-dir",required=True); ap.add_argument("--device",default="cuda"); ap.add_argument("--n-trials",type=int,default=None); args=ap.parse_args()
    tc=load_yaml(args.config); base=load_yaml(tc["base_config"])
    if args.dataset_root: base["data"]["root"]=args.dataset_root
    out=Path(args.run_dir); out.mkdir(parents=True,exist_ok=True)
    study=optuna.create_study(direction="maximize",study_name=tc.get("study_name","gateperceiver"),storage=tc.get("storage"),load_if_exists=True,pruner=optuna.pruners.MedianPruner(n_startup_trials=int(tc.get("n_startup_trials",5)),n_warmup_steps=int(tc.get("n_warmup_epochs",2))))
    ...
    def objective(trial):
        ...
        for path,s in space.items():
            ...
            put(cfg,path,val); chosen[path]=val
        score,_=train_experiment(cfg,out/f"trial_{trial.number:04d}",args.device,trial=trial)
        return score
    n=args.n_trials if args.n_trials is not None else int(tc.get("n_trials",30)); study.optimize(objective,n_trials=n)
    overrides={}
    for path,val in study.best_params.items(): put(overrides,path,val)
    save_yaml(overrides,out/"best_overrides.yaml")
    save_yaml({"best_value":float(study.best_value),"best_trial":int(study.best_trial.number),"best_params":study.best_params},out/"study_summary.yaml")
